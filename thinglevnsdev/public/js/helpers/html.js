define({
    ...
    addBookmarklet: function($el) {
        ...
        // record bookmarklet install event
        $el.on("dragend", function(e) {
            if (e.originalEvent.y < 0) {
                // bookmarklet was dragged outside of the window. Hopefully in the bookmark bar
                if (window._gaq) {
                    window._gaq.push([ '_trackEvent', 'bookmarklet', 'install' ]);
                }
            }
        });
    }
